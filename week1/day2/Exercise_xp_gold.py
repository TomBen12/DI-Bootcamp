...
# milion_list = [el+1 for el in range(0,1000000)]
# print(min(milion_list))
# print(max(milion_list))
# # print(milion_list)
# print(sum(milion_list))
...
# my_str_list = input('enter numbers separated by "," : ').strip().split(',')
# my_list_int = [int(stri) for stri in my_str_list]
# my_tuple_int = tuple(my_list_int)
# print(my_list_int)
# print(my_tuple_int)
...
while True:
    try:
        your_number = int(input('Enter a number from 1-9 including: '))
        if not (1 <= your_number <= 9):
            raise Exception('number not in range')
    except ValueError as v_e:
        print(v_e, 'Value not correct,  try again')
        continue
        ...
        if your_number == random_num:
            print('you win 😜')
            win += 1
        else:
            print('you lose... 😿')
            lost += 1
        want_continue = input('enter anything to continue or enter "q" to quit: ')
        if want_continue.lower() == 'q':
            s_w = 's' if win > 1 else ''
            s_l = 's' if lost > 1 else ''

            print(f'Your score:\n you won: {win} time{s_w}\n you lost {lost} time{s_l}')
            break
        else:
            continue
